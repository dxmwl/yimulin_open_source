...
import com.yimulin.mobile.utils.djUtils.DemoConst
...
//        })
        ...
        //判断是否同意隐私政策
        if (!SpUtils.getBoolean("APP_CONFIG", "AGREE_AGREEMENT", false)) {
            showYszcDialog()
        } else {
            checkLogin()
        }
    ...
    private fun showYszcDialog() {
        ...
        positiveButton.setOnClickListener {
            SpUtils.put("APP_CONFIG", "AGREE_AGREEMENT", true)
            checkLogin()
...
//        initCsjSdk(application)
        ...
        UmengClient.init(application, AppConfig.isLogEnable(), channel)
