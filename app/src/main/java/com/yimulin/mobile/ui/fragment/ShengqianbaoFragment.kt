...
/**
 * @ClassName: ShengqianbaoFragment
 * @Description:
 * @Author: 常利兵
 * @Date: 2024/1/23 0023 15:50
 **/
class ShengqianbaoFragment: AppFragment<AppActivity>() {
    ...
    override fun onClick(view: View) {
        ...
        when (view.id) {
            ...
            R.id.go_buy -> {
                BrowserActivity.start(requireContext(), youhuiUrl)
            }
            R.id.clear_input -> {
            ...
            .request(object : OnHttpListener<HttpData<GetYouhuiApi.GoodsYouhuiDto>> {
                override fun onSucceed(result: HttpData<GetYouhuiApi.GoodsYouhuiDto>?) {
                    ...
                    result?.getData()?.let {
                        ...
                        if (needShare) {
                            XXPermissions.with(this@ShengqianbaoFragment)
