...
import com.yimulin.mobile.http.api.GetFishingCalendarApi
import com.yimulin.mobile.http.api.GetTouchingTheFishDailyApi
import com.yimulin.mobile.http.model.HttpData
import com.yimulin.mobile.ui.dialog.ShareDialog

/**
 * @ClassName: TouchingTheFishDailyFragment
 * @Description:
 * @Author: 常利兵
 * @Date: 2024/1/17 0017 15:10
 **/
class TouchingTheFishDailyFragment: AppFragment<AppActivity>() {
    ...
    override fun initData() {
        getTouchingTheFishDaily()
    }

    private fun getTouchingTheFishDaily() {
        EasyHttp.get(this)
            .api(GetTouchingTheFishDailyApi())
            .request(object : HttpCallback<HttpData<String>>(this) {
                override fun onSucceed(result: HttpData<String>?) {
                    iv_result?.let {
                        Glide.with(this@TouchingTheFishDailyFragment)
                            .load(result?.getData())
                            .into(it)
                    }
                }
